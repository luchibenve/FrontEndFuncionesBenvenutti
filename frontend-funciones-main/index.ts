...
function filterPeople(value: string) {
  const lowerValue = value.toLowerCase();
	// @ts-ignore
	const filteredPeople: Person[] = displayedPeople.filter((p: Person) =>
    p.name.toLowerCase().indexOf(lowerValue) !== -1 ||
    p.birth_year.toLowerCase().indexOf(lowerValue) !== -1 ||
    p.gender.toLowerCase().indexOf(lowerValue) !== -1 ||
    p.url.toLowerCase().indexOf(lowerValue) !== -1
  );
...
function sortPeople(prop: string) {
	...
	// @ts-ignore
	const sortedPeople: Person[] = displayedPeople.toSorted(
		(a: Person, b: Person) => {
			if (sortBy[prop] === "asc") {
